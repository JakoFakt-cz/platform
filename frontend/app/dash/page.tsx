      ...
      <section className="w-full bg-transparent relative flex flex-col items-center justify-center -mt-125 md:px-50 px-10 pb-20">
        <div className="border border-primary shadow-xl rounded-2xl p-5 py-10 bg-secondary px-10 w-full max-w-7xl flex flex-col gap-8">
            ...
            <Image
              src={
                user?.profilePictureUrl
                  ? user.profilePictureUrl
                  : '/images/bozka-dark.svg'
                // TODO: Add DEFAULT profile picture
              }
              alt="Logo projektu"
              width={50}
              height={50}
              className="border border-primary rounded-full aspect-square"
            />
            <h1>Dobrý den, {user?.displayName}!</h1>
          </div>
          <div className="grid grid-cols-3 gap-5">
            ...
            >
              <span>
                Bezpečné ukončení aktuální relace a odhlášení z aplikace.
              </span>
            </DashboardCard>
          </div>
        </div>
