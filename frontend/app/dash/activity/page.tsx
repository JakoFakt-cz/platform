...
import { CommentModel, GetCommentsByAuthor as RetrieveCommentsByAuthor } from '@/actions/comment';
...
export default function ActivityPage() {
  ...
  const [comments, setComments] = useState<{ article: ArticleModel, comments: CommentModel[] }[]>([]);
  const userId = '69492c68e2b63e716b2dd9d1'; // TODO: replace with real user ID

  useEffect(() => {
    const fetchData = async () => {
      const articles = await RetrieveArticlesFromBackend({ limit: 50, latest: true, author: userId });
      setArticles(articles);

      const comments = await RetrieveCommentsByAuthor({ authorId: userId, limit: 50 });
                    ...
                    <div className="mt-4 flex flex-col gap-4">
                      {articles.length == 0 ? (
                        ...
                        articles.map((article, index) => {
                          return (
                            <Link href={`/article?id=${article._id}`} key={index}>
                              <div className="p-4 border border-primary/10 rounded-lg">
                                <h3 className="font-bold">{article.header.title}</h3>
                                <p className="text-sm text-primary/60">Vytvořeno: {FormatWhenMessage(new Date(article.createdAt))}</p>
                              </div>
                            </Link>
                          );
                        })
                    ...
                    <div className="mt-4 flex flex-col gap-4">
                      {comments.length == 0 ? (
                        ...
                        comments.map((result, index) => {
                          const { _id, header } = result.article;
                          const { title } = header;

                          return (
                            <div key={index} className='flex flex-col gap-4'>
                              {result.comments.map((comment, index2) => {

                                return (
                                  <Link href={`/article#${comment._id}?id=${_id}`} key={index2}>
                                    <div className="p-4 border border-primary/10 rounded-lg">
                                      <p className="text-primary/80">"{comment.content}"</p>
                                      <p className="text-sm text-primary/60">Okomentováno u příspěvku "{title}" dne {FormatWhenMessage(new Date(comment.createdAt))}</p>
                                    </div>
                                  </Link>
                                );
                              })}
